...
class Episode(Base):
    ...
    user_id = Column(String, ForeignKey("users.id"), nullable=False) # ユーザーへのリンクも直接持つと便利
    ...
    emotion_analysis = Column(JSON) # { "primary_emotion": "joy", ... }
    keywords = Column(JSON) # ["keyword1", "keyword2"]
    topics = Column(JSON) # ["topic1", "topic2"]
    named_entities = Column(JSON) # [{ "text": "...", "type": "PERSON" }, ...]
    summarization = Column(JSON) # { "short_summary": "...", "key_points": [...] }

    # --- トラウマ・イベント関連情報 ---
    is_trauma_event = Column(Boolean, default=False)
    trauma_event_details = Column(JSON) # is_trauma_eventがtrueの場合に格納

    # --- ユーザーによる評価・状態管理 ---
    user_importance_rating = Column(String, default="not_set")
    user_labels_or_tags = Column(JSON)
